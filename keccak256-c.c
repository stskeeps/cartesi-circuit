...
/**
  *  Function to compute SHA3-256 on the input message. The output length is fixed to 32 bytes.
  */
void Keccak_SHA256(const unsigned char *input, unsigned int inputByteLen, unsigned char *output)
{
    Keccak(1088, 512, input, inputByteLen, 0x01, output, 32);
}
...
{
    unsigned int round = 0, x = 0, y = 0, j = 0, t = 0;
...
int Keccak256_64(uint8 *input, uint8 *expected)
{
    uint8_t output[32];
    
    Keccak(1088, 512, input, 64, 0x01, output, 32);
